...
def _active_agents(conn: sqlite3.Connection, thread_id: str) -> list[str]:
    ...
    if row is None:
        return ["main", "researcher", "planner", "coder"]
...
async def _queue_depth() -> dict[str, int]:
    ...
    for item in payload:
        ...
        if isinstance(ready, int):
            total_ready += max(0, ready)
        if isinstance(unacked, int):
            total_unacked += max(0, unacked)
    return {"messages_ready": total_ready, "messages_unacknowledged": total_unacked}
...
async def maybe_execute_command(
    ...
    if command == "/group" and len(args) >= 2 and args[0] in {"on", "off"}:
        known_agents = {"main", "researcher", "planner", "coder"}
        agent = args[1]
        ...
        current = [a for a in current if a in known_agents]
    ...
    if command == "/compact":
        try:
            ...
            return "compaction unavailable (broker down)"
    ...
    if command == "/status":
        ...
        worker_health: dict[str, object] = {"reachable": False, "workers": []}
        try:
            ...
            worker_health = {"reachable": len(worker_names) > 0, "workers": worker_names}
        except Exception as exc:
            _logger.warning("Celery ping failed: %s", exc)
            worker_health = {"reachable": False, "error": str(exc)}
        return json.dumps(
            {
                "providers": providers,
                "queues": queue_depth,
