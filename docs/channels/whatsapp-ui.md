...
- Status view includes last disconnect diagnostics (`disconnect_code`, `disconnect_reason`) when available.
- Disconnect action.
...
- Status `close` with disconnect `401 (loggedOut)`: session auth was invalidated; use `Force Re-pair`,
  then `Load QR` and complete pairing.
