      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.15fr_1fr]">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <MessagesSquare className="h-3.5 w-3.5 text-text3" />
                ...
                <div className="mt-1 flex items-center justify-between pl-5">
                  <span className="truncate text-xs text-text3">{item.id}</span>
                  <span className="ml-2 shrink-0 text-xs text-text3">
                    {formatTimestampHuman(item.updated_at)}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-4">
          <Card
            header={
              <div className="flex items-center gap-2">
                <MessagesSquare className="h-4 w-4 text-text3" />
                <span className="font-mono text-xs font-semibold uppercase tracking-widest text-text2">
                  Thread Detail
                </span>
              </div>
          ...
          >
            {!selectedThreadId ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <MessagesSquare className="mb-3 h-10 w-10 text-text3" />
                <p className="text-sm text-text3">
                  Select a thread to view details
                </p>
              </div>
            ) : detail.isLoading ? (
              <p className="py-6 text-center text-sm text-text3">Loading...</p>
            ) : detail.data ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className="text-xs text-text3">Thread ID</span>
                    <p className="mt-0.5 truncate text-sm font-medium text-text">
                      {detail.data.id}
                    </p>
                  </div>
                  <div>
                    <span className="text-xs text-text3">Channel</span>
              ...
              <div className="max-h-[28rem] space-y-2 overflow-auto">
                {messageItems.length === 0 ? (
                  ...
                  messageItems.map((m) => (
                    <div
                      key={m.id}
                      className="rounded-lg border border-[var(--color-border)] bg-surface-2 px-3 py-2"
                    >
                      <div className="mb-1 flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-text3">
                          {formatTimestampHuman(m.created_at)}
                        </span>
                      </div>
